...
- `synapse-sdk` 의존성 `2026.2.8` → `2026.2.9` 업그레이드. plugin 자체 코드는 무변경이다.
  - SDK 2026.2.9: **대용량 파일이 다 올라간 뒤 타임아웃으로 버려지던 것을 고친다** (SYN-7658). v1 클라이언트의 read timeout 이 15초인데 chunked upload 의 finalize 는 backend 가 다중 GB 파일을 조립·검증하는 구간이라 실측 25~31초(6~11 GB)가 걸린다. 클라이언트가 먼저 끊으면 후속 `POST data_files/` 를 보내지 않아 바이트는 다 올라갔는데 데이터 파일도 데이터 유닛도 만들어지지 않았다 — production 회차 하나에서 12건 중 11건이 그렇게 사라졌다.
  - 이제 전송 구간에 전용 read timeout(기본 300초)이 붙고, 그럼에도 응답이 끊기면 `GET chunked_upload/{id}/` 로 되물어 **재전송 없이** 이어 간다. 조정은 실행 파라미터 `extra_params.upload_read_timeout`(초).
  - 일부 파일만 올라간 회차가 결과에 드러난다 — `partial_failure` / `lost_files` 와 경고 로그. 종전에는 한 건만 성공해도 그냥 성공으로 끝나 유실을 읽을 방법이 없었다.
  - **5 GB 이상 파일을 올리는 임포트는 이 버전부터 쓸 것.** 그 아래에서는 종전과 동작이 같다.
